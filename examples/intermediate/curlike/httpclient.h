...
public:
    ...
    bool Connect(const std::string &host, int port, const std::string &ip, qor::io::network::sockets::eAddressFamily address_family, qor::io::network::addrinfo_flags socket_flags, bool tcp_nodelay, bool ipv6_v6only, time_t timeout_sec)
    {
        qor_pp_ofcontext;
        m_client.Configure(host, port, ip, address_family, socket_flags, tcp_nodelay, ipv6_v6only, timeout_sec);
        return Connect();
