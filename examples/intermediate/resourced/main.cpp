...
{    
    ...
    make_runable(
        ...
        {            
            ...
            resourceHub->SubscribeForAllByPath("F:/Develop/", &HandlePathUpdate);
            Path fontPath("F:/Develop/thorvg-1.0-pre33/thorvg-1.0-pre33/examples/resources/font");
            resourceHub->AddPath(fontPath);
            char c;
            std::cin >> c;
            return 0;
        }
...
bool HandlePathUpdate(Resource* res, ResourceStatus status)
{
    if(res)
    {
        ...
        if(res->Type() == res::JSON::StaticType() && status == Named)
        {
            res->Locate();
            return true;
        }

        if(res->Type() == res::JSON::StaticType() && status == Located)
        {
            res->Claim();
            return true;
        }

        if(res->Type() == res::JSON::StaticType() && status == Claimed)
        {
            res::JSON* json = dynamic_cast<res::JSON*>(res);
            if(json)
            {
                json->GetObject();
            }
