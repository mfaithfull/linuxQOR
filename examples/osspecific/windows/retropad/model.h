...
public:
    ...
    qor::pipeline::PODBuffer<qor::platform::win::TCHAR>& Document();
    void InsertTimeDate();
    void DoFileNew();    
    bool LoadDocumentFromPath(stdstring path);
    bool FindInText(const qor::platform::win::TCHAR* needle, bool matchCase, bool searchDown, unsigned long startPos, unsigned long *outStart, unsigned long *outEnd);
    int ReplaceAllOccurrences(bool matchCase);
    bool Save(const stdstring& path);
    bool SaveTextFile(const stdstring& path, const qor::platform::win::TCHAR* text, size_t length, TextEncoding encoding);
...
private:
    ...
    qor::pipeline::PODBuffer<qor::platform::win::TCHAR> m_document;
};
