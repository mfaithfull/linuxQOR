...
ViewState* ViewState::Create(void* instance, const TCHAR* windowClass, const qor::platform::win::Menu& menu, ModelState* model, void* param)
{
    ...
    view->hwndMain = new qor::platform::win::Window(windowClass, APP_TITLE.c_str(), WS_OVERLAPPEDWINDOW, 0, CW_USEDEFAULT, CW_USEDEFAULT, DEFAULT_WIDTH, DEFAULT_HEIGHT, NULL, menu, (HINSTANCE)(view->m_instance), param);
...
void ViewState::CreateEditControl()
{
    ...
    qor::platform::win::Menu menu(1);
    hwndEdit = new qor::platform::win::Window( TEXT("EDIT"), nullptr, style, WS_EX_CLIENTEDGE, 0, 0, 0, 0, hwndMain, menu, m_instance, nullptr);
...
{
    qor::platform::win::Menu menu(hwndMain->GetMenu());    
...
void ViewState::ToggleStatusBar(bool visible)
{
    ...
    if (visible) 
    {
        if (!hwndStatus) 
        {
            hwndStatus = new qor::platform::win::Window(CreateStatusWindow(WS_CHILD | SBARS_SIZEGRIP, TEXT(""), (HWND)(hwndMain->GetHandle().Use()), 2));
        }
...
void ViewState::ShowFindDialog()
{
    ...
    hFindDlg = new qor::platform::win::Window(FindText(reinterpret_cast<LPFINDREPLACE>(&find)));
}
...
void ViewState::ShowReplaceDialog() 
{
    ...
    find.Flags = model->GetFindFlags();
    hReplaceDlg = new qor::platform::win::Window(ReplaceText(reinterpret_cast<LPFINDREPLACE>(&find)));
}
...
void ViewState::DoSelectFont()
{
    ...
    if (ChooseFont(&cf)) 
    {
        ...
        if (newFont) 
        {
            ...
            hFont = new qor::platform::win::Font(newFont);
            ApplyFontToEdit();
            UpdateLayout();
        }
    }
...
void ViewState::HandleFindReplace(qor::platform::win::FindReplace* lpfr)
{
...
int ViewState::Run()
{
    qor::platform::win::AcceleratorTable accTable(LoadAccelerators((HINSTANCE)(m_instance), MAKEINTRESOURCE(IDC_RETROPAD)));
    qor::platform::win::Message msg;
