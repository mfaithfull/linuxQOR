...
qor_issue provies a general purposd issue template and allows for the collection of contextual information about when and where the issue was raised. The details of what the issue is are left to be added by extension.
...
continuable a real error but execution can carry on, if it's handled
...
and then cause you program to terminate. This will happen for Warning or above. Notes will just be 
...
Important       things that are significant, unexpected or off the happy path, missing resources, exceptions, failures
