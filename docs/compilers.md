Wow, the C++ compiler market has narrowed since this project began. We need a modernish C++ compiler and that means we'll be supporting Microsoft's cl, known as MSVC and g++ known as GNU C++/GCC. Clang from Apple & Google effectively emulates the front ends of both while using an LLVM backend so It'll be best efforts to support that as well.

msvc
gcc
clang

The qor_compiler library may eventually be a QOR module
