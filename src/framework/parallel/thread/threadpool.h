...
namespace qor { namespace thread{
    ...
    public:
        ...
        qor_pp_module_interface(QOR_THREAD) virtual void Setup();
        qor_pp_module_interface(QOR_THREAD) virtual void Shutdown();
        qor_pp_module_interface(QOR_THREAD) virtual ~ThreadPool() noexcept;                                                                //Destruct the thread pool. Waits for all tasks to complete, then destroys all threads. If a cleanup function was set, it will run in each thread right before it is destroyed. Note that if the pool is paused, then any tasks still in the queue will never be executed.
        ...
        qor_pp_module_interface(QOR_THREAD) void Purge();
        ...
        qor_pp_module_interface(QOR_THREAD) void Reset();
        ...
        qor_pp_module_interface(QOR_THREAD) void Reset(const std::size_t num_threads);
        ...
        qor_pp_module_interface(QOR_THREAD) void Wait();
