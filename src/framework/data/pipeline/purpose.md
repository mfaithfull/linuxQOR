...
In Pull mode calls flow from sink to source, i.e. the pipeline pumps by calling the sink which calls the source to read data as required
In Push mode calls flow from source to sink, i.e. the pipeline pumps by calling the source which calls the sink to write data as it's available.
...
Both the sink and the source see the inline filter as a simple buffer 
