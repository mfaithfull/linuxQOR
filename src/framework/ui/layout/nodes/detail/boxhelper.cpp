...
namespace qor{ namespace ui{ namespace layout { namespace detail {
    ...
    namespace boxhelper
    {
        namespace 
        {
            int64_t SafeRatio(int value, int numerator, int denominator) 
            {
            ...
            void ComputeGrow(std::vector<Element>* elements, int extra_space, int flex_grow_sum) 
            {
                ...
                {
                    const int added_space = static_cast<int>(SafeRatio(extra_space, element.flex_grow, flex_grow_sum));
            ...
            void ComputeShrinkEasy(std::vector<Element>* elements, int extra_space, int flex_shrink_sum) 
            {
                for (Element& element : *elements) 
                {
                    const int added_space = static_cast<int>(SafeRatio(extra_space, element.min_size * element.flex_shrink, flex_shrink_sum));
                    extra_space -= added_space;
            ...
            void ComputeShrinkHard(std::vector<Element>* elements, int extra_space, int size) 
            {
                for (Element& element : *elements) 
                {
                    ...
                    const int added_space = static_cast<int>(SafeRatio(extra_space, element.min_size, size));
                    extra_space -= added_space;
