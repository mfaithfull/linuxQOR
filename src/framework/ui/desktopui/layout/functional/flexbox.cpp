...
namespace qor{ namespace ui{ namespace layout {
    ...
    namespace 
    {
        ...
        class qor_pp_module_interface(QOR_DESKTOPUI) Flexbox : public Node 
        {
        public:
            ...
            void Layout(flexboxhelper::Global& global, bool compute_requirement = false) 
            {
                global.blocks.reserve(children_.size());
                for (auto& child : children_) 
                {
                    flexboxhelper::Block block;
            ...
            flexboxhelper::Global global_;
        };

    }//

    //A container displaying elements on row/columns and capable of
    ...
    {
        return new_ref<Flexbox>(std::move(children), config);
    }

    //A container displaying elements in rows from left to right. When
