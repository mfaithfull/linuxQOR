...
namespace qor{ namespace ui{ namespace layout { namespace flexboxhelper {
    namespace 
    {
        ...
        void SetX(Global& global) 
        {
            ...
            {
                std::vector<ui::layout::detail::boxhelper::Element> elements;
                elements.reserve(line.blocks.size());
                for (auto* block : line.blocks) 
                {
                    ui::layout::detail::boxhelper::Element element;
                    element.min_size = block->min_size_x;
                    element.flex_grow = block->flex_grow_x != 0 || global.config.justify_content == FlexboxConfig::JustifyContent::Stretch ? 1 : 0;
                    element.flex_shrink = block->flex_shrink_x;
                    elements.push_back(element);
                }

                ui::layout::detail::boxhelper::Compute(&elements, global.size_x - global.config.gap_x * (int(line.blocks.size()) - 1));
        ...
        void SetY(Global& g) 
        {
            ...
            ui::layout::detail::boxhelper::Compute(&elements, 10000);
            ...
            switch (g.config.align_content) 
            {
                ...
                {
                    for (int i = static_cast<int>(ys.size()) - 1; i >= 1; --i) 
                    {
                        ys[i] += remaining_space;
