...
namespace 
{
    ...
    using WBP = qor::ui::tty::WordBreakProperty;
    ...
    constexpr auto g_extend_characters{
        []() constexpr 
        {
    ...
    {
        if (ucs < table.front().first || ucs > table.back().last) 
        {
            return false;
        }
    ...
    {
        if (qor::ui::tty::IsControl(ucs)) 
        {
            return -1;
        }

        if (qor::ui::tty::IsCombining(ucs)) 
        {
            return 0;
        }

        if (qor::ui::tty::IsFullWidth(ucs)) 
        {
            return 2;
        }
