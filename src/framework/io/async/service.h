...
namespace qor { namespace io{ namespace async{
    ...
    class qor_pp_module_interface(QOR_ASYNCIOSERVICE) Service : public app::IFeature
    {
    public:
        ...
        ref_of<Context::Session>::type GetSession();

    private:

        ref_of<thread::ThreadPool>::type m_threadPool;
        unsigned m_contextCount;
        ref_of<Context>::type* m_contextArray;
        size_t m_contextIndex;

    };
    }} //io::async

    qor_pp_declare_instancer_of(io::async::Service, SingletonInstancer);
    qor_pp_declare_factory_of(io::async::Service, ExternalFactory);
    constexpr GUID AsyncIOServiceGUID = {0x6201abca, 0xf405, 0x4709, {0xa9, 0x86, 0x26, 0x82, 0xeb, 0x66, 0xfd, 0xc6}};
    qor_pp_declare_guid_of(io::async::Service,AsyncIOServiceGUID);
