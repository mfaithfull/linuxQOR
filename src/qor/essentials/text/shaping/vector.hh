...
#ifndef QOR_PP_H_TEXT_SHAPING_VECTOR
#define QOR_PP_H_TEXT_SHAPING_VECTOR

#include "array.hh"

namespace qor{ namespace qb{

    template <typename Type, bool sorted=false>
    struct vector_t
    {
        ...
        vector_t (std::initializer_list<Type> lst) : vector_t ()
        {
            alloc (lst.size (), true);
            for (auto&& item : lst)
            push (item);
        ...
        explicit vector_t (const Iterable &o) : vector_t ()
        {
            extend (o, true);
        }

        
        vector_t (const vector_t &o) : vector_t ()
        {
        ...
        template <unsigned n>
        void set_storage (Type (&array)[n])
        { set_storage (array, n); }

        void set_storage (hb_array_t<Type> array)
        { set_storage (array.arrayZ, array.length); }

        template <typename T = Type, hb_enable_if (hb_is_trivially_constructible(T) && hb_is_trivially_destructible(T))>
        void set_storage (Type *array, unsigned n)
        {
            assert (allocated == 0);
            assert (length == 0);
    ...
    public:
        ...
        * Returns nullptr if storage is not owned by this vector. */
        Type* steal (unsigned *len = nullptr, int *allocated_out = nullptr)
        {
            if (!is_owned ())
            return nullptr;
            if (len)
        ...
        void recycle_buffer (Type *buffer, unsigned len, int allocated_len)
        {
            fini ();
            arrayZ = buffer;
            length = len;
        ...
        friend void swap (vector_t& a, vector_t& b) noexcept
        {
        ...
        vector_t& operator = (vector_t &&o) noexcept
        {
            hb_swap (*this, o);
            return *this;
        }
        ...
        bool operator == (const vector_t &o) const { return as_array () == o.as_array (); }

        bool operator != (const vector_t &o) const { return !(*this == o); }
        ...
        // Sink interface.
        template <typename T>
        vector_t& operator << (T&& v) { push (std::forward<T> (v)); return *this; }
        ...
        template <typename T = Type, hb_enable_if (hb_is_trivially_copy_assignable(T))>
        ...
        // Specialization for vector_t<hb_{vector,array}_t<U>> to speed up.
        template <typename T = Type, hb_enable_if (hb_is_same (T, vector_t<typename T::item_t>) || hb_is_same (T, hb_array_t <typename T::item_t>))>
        void grow_vector (unsigned size, hb_priority<1>)
        {
            hb_memset (arrayZ + length, 0, (size - length) * sizeof (*arrayZ));
            length = size;
        }

        template <typename T = Type, hb_enable_if (hb_is_trivially_copyable (T))>
        void copy_array (hb_array_t<Type> other)
        {
        ...
        void copy_array (hb_array_t<const Type> other)
        {
            hb_memcpy ((void *) (arrayZ + length), (const void *) other.arrayZ, other.length * item_size);
            length += other.length;
        }
        ...
        template <typename T = Type,
        hb_enable_if (!hb_is_trivially_copyable (T) && !std::is_copy_constructible<T>::value && std::is_default_constructible<T>::value && std::is_copy_assignable<T>::value)>
        void copy_array (hb_array_t<const Type> other)
        {
            for (unsigned i = 0; i < other.length; i++)
            {
        ...
        void shift_down_vector (unsigned i)
        {
            for (; i < length; i++)
        ...
        bool duplicate_vector_from_pool (allocator_t *allocator, const vector_t &other)
        {
        ...
        template <bool Sorted = sorted, hb_enable_if (!Sorted)>
    ...
    using hb_sorted_vector_t = vector_t<Type, true>;

}}

#endif//QOR_PP_H_TEXT_SHAPING_VECTOR