...
#ifndef QOR_PP_H_TEXT_SHAPING_BUFFER
#define QOR_PP_H_TEXT_SHAPING_BUFFER

#include "common.h"
#include "src/qor/essentials/text/unicode/scripts.h"
...
namespace qor{ namespace qb{
    ...
    struct buffer_t
    {
        ...
        codepoint_t not_found_variation_selector; /* HB_CODEPOINT_INVALID or something else. */
        ...
        * Index 0 is for "pre-context", 1 for "post-context". */
        ...
        typedef void (*changed_func_t) (buffer_t *buffer, void *user_data);
        ...
        HB_ALWAYS_INLINE glyph_info_t &cur (unsigned int i = 0) { return info[idx + i]; }

        HB_ALWAYS_INLINE glyph_info_t cur (unsigned int i = 0) const { return info[idx + i]; }

        HB_ALWAYS_INLINE glyph_position_t &cur_pos (unsigned int i = 0) { return pos[idx + i]; }

        HB_ALWAYS_INLINE glyph_position_t cur_pos (unsigned int i = 0) const { return pos[idx + i]; }

        HB_ALWAYS_INLINE glyph_info_t &prev ()      { return out_info[out_len ? out_len - 1 : 0]; }

        HB_ALWAYS_INLINE glyph_info_t prev () const { return out_info[out_len ? out_len - 1 : 0]; }
        ...
        void similar (const buffer_t &src);
        void reset ();
        void clear ();
        ...
        bool verify(buffer_t* text_buffer, font_t* font, const feature_t* features, unsigned int num_features, const char * const *shapers)
        ...
        void add (codepoint_t  codepoint, unsigned int cluster);
        void add_info (const glyph_info_t &glyph_info);
        void add_info_and_pos (const glyph_info_t &glyph_info, const glyph_position_t &glyph_pos);

        void reverse_range (unsigned start, unsigned end)
        {
            hb_array_t<glyph_info_t> (info, len).reverse (start, end);
            if (have_positions)
            hb_array_t<glyph_position_t> (pos, len).reverse (start, end);
        }
        ...
        * If there's no output, just advance idx. */
        ...
        void merge_out_clusters_impl (unsigned int start, unsigned int end);
        /* Merge clusters for deleting current glyph, and skip it. */

        void delete_glyph ();
        void delete_glyphs_inplace (bool (*filter) (const glyph_info_t *info));
        ...
        * If interior is true, then the cluster having the minimum value is skipped. */
        void _set_glyph_flags_impl (mask_t mask,
        unsigned start,
        unsigned end,
        bool interior,
        ...
        bool message_impl (hb_font_t *font, const char *fmt, va_list ap) HB_PRINTF_FUNC(3, 0);
        void changed ();

        static void set_cluster (glyph_info_t& inf, unsigned int cluster, unsigned int mask = 0)
        {
        ...
        void _infos_set_glyph_flags(glyph_info_t* infos, unsigned int start, unsigned int end, unsigned int cluster, mask_t mask)
        {
            if (start == end)
            return;
            ...
            if (cluster_level == HB_BUFFER_CLUSTER_LEVEL_CHARACTERS || (cluster != cluster_first && cluster != cluster_last))
            {
        ...
        unsigned _infos_find_min_cluster (const glyph_info_t *infos, unsigned start, unsigned end, unsigned cluster = UINT_MAX)
        {
            if (start == end)
            return cluster;
...
#endif//QOR_PP_H_TEXT_SHAPING_BUFFER