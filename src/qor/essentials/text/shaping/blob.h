...
#ifndef QOR_PP_H_TEXT_SHAPING_BLOB
#define QOR_PP_H_TEXT_SHAPING_BLOB
...
namespace qor{ namespace qb{
    ...
    struct blob_t
    {
        ...
        void replace_buffer(const char* new_data, unsigned new_length, hb_memory_mode_t new_mode, void* new_user_data, destroy_func_t new_destroy)
        {
            if (new_data != data)
            destroy_user_data();
            data = new_data;
    ...
    public:
        ...
        destroy_func_t destroy = nullptr;

        /*
        * Blob-recycling helpers.
        *
        * Encoders that produce a sized byte buffer and wrap it as an
        * blob_t can amortize malloc/blob-allocation across repeated
        ...
        * the same blob_t handle is returned, skipping malloc/free
        ...
        * nullptr on allocation failure. */
        static inline char *
        recycle_acquire (blob_t *recycled, unsigned   needed, unsigned  *out_capacity, char     **out_replaced_buf)
        {
            *out_replaced_buf = nullptr;
            ...
            if (recycled && recycled->destroy == recycle_data_destroy)
            {
                ...
                if (unlikely (hb_unsigned_add_overflows (needed, needed / 2, &alloc_bytes)))
                alloc_bytes = needed;
        ...
        /* Wrap @buf (of @capacity, with @length used) into an blob_t.
        ...
        * recycle_acquire(). */
        ...
        recycle_finalize (char* buf, unsigned capacity, unsigned length, blob_t *recycled, char* replaced_recycled_buf)
        {
            if (recycled && recycled->destroy == recycle_data_destroy)
            {
            auto *bd = (recycle_data_t *) recycled->user_data;
            ...
            * destroy closure so the next recycle round can reuse it. */
            ...
            return hb_blob_create ((const char *) buf, length, HB_MEMORY_MODE_WRITABLE, bd, recycle_data_destroy);
        }
        ...
        * recycled buffer untouched. */
        static inline void recycle_abort (char *buf, blob_t *recycled)
        {
            if (!buf) return;
            if (recycled && recycled->destroy == recycle_data_destroy)
            {
        ...
        * "drop"). */
        static inline void
        recycle_stash (blob_t **slot, blob_t *blob)
        {
    ...
    struct blob_ptr_t
    {
        ...
        blob_ptr_t (blob_t *b_ = nullptr) : b (b_) {}
        blob_t * operator = (blob_t *b_) { return b = b_; }
        const T * operator -> () const { return get (); }
        const T & operator * () const  { return *get (); }
        template <typename C> operator const C * () const { return get (); }
        operator const char * () const { return (const char *) get (); }
        const T * get () const { return b->as<T> (); }
        blob_t * get_blob () const { return b.get_raw (); }
        unsigned int get_length () const { return b.get ()->length; }
        void destroy () { hb_blob_destroy (b.get_raw ()); b = nullptr; }

        private:
        hb_nonnull_ptr_t<blob_t> b;
    };

}}

#endif//QOR_PP_H_TEXT_SHAPING_BLOB