...
#ifndef QOR_PP_H_DATASTRUCTURES_FROZEN_UNORDERED_SET
#define QOR_PP_H_DATASTRUCTURES_FROZEN_UNORDERED_SET
...
namespace qor{ namespace frozen { namespace bits {
    ...
    {
        static constexpr std::size_t storage_size = bits::next_highest_power_of_two(N) * (N < 32 ? 2 : 1); // size adjustment to prevent high collision rate for small sets
    ...
    public:
        ...
        constexpr unordered_set(container_type keys, Hash const &hash, KeyEqual const &equal)
            : KeyEqual{equal}
            , keys_{keys}
            , tables_{bits::make_pmh_tables<storage_size>(keys_, hash, equal, bits::Get{}, default_prg_t{})} 
        { }
...
#endif//QOR_PP_H_DATASTRUCTURES_FROZEN_UNORDERED_SET
