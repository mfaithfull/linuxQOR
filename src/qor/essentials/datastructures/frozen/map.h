...
#ifndef QOR_PP_H_DATASTRUCTURES_FROZEN_MAP
#define QOR_PP_H_DATASTRUCTURES_FROZEN_MAP
...
namespace impl {
    ...
    public:
        ...
        constexpr auto operator()(std::pair<Key1, Value> const &self, std::pair<Key2, Value> const &other) const -> 
            decltype(key_comp()(std::get<0>(self), std::get<0>(other))) 
        {
            return key_comp()(std::get<0>(self), std::get<0>(other));
        }

        template <class Key1, class Key2, class Value>
        constexpr auto operator()(Key1 const &self_key, std::pair<Key2, Value> const &other) const -> 
            decltype(key_comp()(self_key, std::get<0>(other))) 
        {
            return key_comp()(self_key, std::get<0>(other));
        }

        template <class Key1, class Key2, class Value>
        constexpr auto operator()(std::pair<Key1, Value> const &self, Key2 const &other_key) const -> 
            decltype(key_comp()(std::get<0>(self), other_key)) 
        {
            return key_comp()(std::get<0>(self), other_key);
        }

        template <class Key1, class Key2>
        constexpr auto operator()(Key1 const &self_key, Key2 const &other_key) const -> 
            decltype(key_comp()(self_key, other_key)) 
        {
    ...
    public:
        ...
        constexpr map(std::initializer_list<value_type> items, Compare const &compare) : map{container_type {items}, compare} 
        { }
    ...
    public:
        ...
        constexpr map(std::initializer_list<value_type>, Compare const &compare) : impl::CompareKey<Compare>{compare} 
        { }
...
#endif//QOR_PP_H_DATASTRUCTURES_FROZEN_MAP
