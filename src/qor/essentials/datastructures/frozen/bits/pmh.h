...
#ifndef QOR_PP_H_DATASTRUCTURES_FROZEN_BITS_PMH
#define QOR_PP_H_DATASTRUCTURES_FROZEN_BITS_PMH
...
namespace qor { namespace frozen { namespace bits {
    ...
    struct pmh_tables : private Hasher 
    {
        ...
        constexpr std::size_t lookup(const KeyType & key, const HasherType& hasher) const 
        {
            ...
            if (!d.is_seed()) 
            { 
                return static_cast<std::size_t>(d.value()); // this is narrowing std::uint64 -> std::size_t but should be fine
            }
            else 
            {
...
#endif//QOR_PP_H_DATASTRUCTURES_FROZEN_BITS_PMH
