...
#ifndef QOR_PP_H_DATASTRUCTURES_FROZEN_BITS_BASIC_TYPES
#define QOR_PP_H_DATASTRUCTURES_FROZEN_BITS_BASIC_TYPES
...
namespace qor { namespace frozen { namespace bits {
    ...
    class carray 
    {
        ...
        constexpr carray(const T& value, std::index_sequence<I...>) : data_{((void)I, value)...} { }
    ...
    public:
        ...
        constexpr carray(const value_type& val) : carray(val, std::make_index_sequence<N>()) { }

        template <typename U, std::enable_if_t<std::is_convertible<U, T>::value, std::size_t> M>
        constexpr carray(U const (&init)[M]) : carray(init, std::make_index_sequence<N>())
        {
            static_assert(M >= N, "Cannot initialize a carray with an smaller array");
        }

        template <typename U, std::enable_if_t<std::is_convertible<U, T>::value, std::size_t> M>
        constexpr carray(std::array<U, M> const &init) : carray(init.begin(), std::make_index_sequence<N>())
        {
            static_assert(M >= N, "Cannot initialize a carray with an smaller array");
        }

        template <typename U, std::enable_if_t<std::is_convertible<U, T>::value>* = nullptr>
        constexpr carray(std::initializer_list<U> init) : carray((check_initializer(init), init.begin()), std::make_index_sequence<N>())
        { }

        template <typename U, std::enable_if_t<std::is_convertible<U, T>::value>* = nullptr>
        constexpr carray(const carray<U, N>& rhs) : carray(rhs.begin(), std::make_index_sequence<N>())
        { }
...
#endif//QOR_PP_H_DATASTRUCTURES_FROZEN_BITS_BASIC_TYPES
