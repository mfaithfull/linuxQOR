...
#ifndef QOR_PP_H_MPL_ALGORITHM_COMBINATIONS
#define QOR_PP_H_MPL_ALGORITHM_COMBINATIONS
...
namespace qor { namespace mpl { namespace detail {
        ...
        struct combinations_single_impl<1> 
        {
            ...
            using f = detail::rlist<list<call<join<F>, list<T0>, PrevTypes, list<Ts...>>>, typename combinations_single_impl<combinations_single_impl_select(sizeof...(Ts))>::template f<F,typename join<>::template f<PrevTypes, list<T0>>,Ts...>>;
        };
    ...
    using combinations =
            if_<size<push_back<uint_<0>, greater_than<>>>,
                detail::combinations_single<detail::combinations_impl<(N::value - 1), FrontC, FullC>, join<C>>, always<list<>, C>>;
}}//qor::mpl

#endif//QOR_PP_H_MPL_ALGORITHM_COMBINATIONS
