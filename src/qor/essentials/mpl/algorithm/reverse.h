...
#ifndef QOR_PP_H_MPL_ALGORITHM_REVERSE
#define QOR_PP_H_MPL_ALGORITHM_REVERSE
...
namespace qor { namespace mpl { namespace detail {
        ...
        struct reverse_impl<2> 
        {
            ...
            using f = typename reverse_impl<select_reverse(sizeof...(Ts))>::template f<rlist<list<T1, T0>, Tail>, Ts...>;
        };

        template <>
        struct reverse_impl<4> {
            template <typename Tail, typename T0, typename T1, typename T2, typename T3, typename... Ts>
    ...
    //Reverses the order of elements in a pack
    template <typename C = listify>
    struct reverse 
    {
        template <typename... Ts>
        using f = typename detail::recursive_join<C>::template f<
                typename detail::reverse_impl<detail::select_reverse(sizeof...(Ts))>::template f<detail::rlist_tail_of8, Ts...>>;
    };
...
#endif//QOR_PP_H_MPL_ALGORITHM_REVERSE
