...
#include <type_traits>
#include "../mpl/bool.h"
#include "../mpl/functional/identity.h"
...
namespace qor{ namespace mpl {
    ...
    // tests if a type is an integral type
    ...
    // tests if a type is an lvalue reference
    ...
    // tests if a type is an rvalue reference
    ...
    {
        template <typename T>
        using f = typename C::template f<bool_<std::is_trivially_default_constructible<T>::value>>;
    };
    ...
    {
        template <typename T>
        using f = typename C::template f<bool_<std::is_nothrow_default_constructible<T>::value>>;
    };
