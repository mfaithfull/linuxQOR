...
namespace qor{ namespace log {
    ...
    class qor_pp_module_interface(QOR_LOG) Important :  public Log
    {
    public:
        Important(const std::string& message);
        Important(const Important& src);
        virtual ~Important() noexcept;
    ...
    qor_pp_module_interface(QOR_LOG) void important(const std::string& message);
