...
namespace qor{ namespace reflection {
    ...
    //     std::array<int, qor::reflection::tuple_size<my_structure>::value > a;
    template <class T>
    using tuple_size = detail::size_t_< qor::reflection::detail::fields_count<T>() >;


    // `tuple_size_v` is a template variable that contains fields count in a T and
    // works for any T that satisfies \aggregate.
    //     std::array<int, qor::reflection::tuple_size_v<my_structure> > a;
    template <class T>
