...
#ifndef QOR_PP_H_REFLECTION_DETAIL_CORE14_CLASSIC
#define QOR_PP_H_REFLECTION_DETAIL_CORE14_CLASSIC
...
#   pragma warning( disable : 4554 )
        ...
        constexpr std::size_t type_to_id_extension_apply(std::size_t ext) noexcept 
        {
            ...
            static_assert(
                ...
                "====================> QOR Reflection: Too many extensions for a single field (something close to `int************************** p;` is in the POD type)."
            );
...
#define qor_pp_refl_register_type(Type, Index)                  \
        ...
        qor_pp_refl_register_type(unsigned char         , 1)
        qor_pp_refl_register_type(unsigned short        , 2)
        qor_pp_refl_register_type(unsigned int          , 3)
        qor_pp_refl_register_type(unsigned long         , 4)
        qor_pp_refl_register_type(unsigned long long    , 5)
        qor_pp_refl_register_type(signed char           , 6)
        qor_pp_refl_register_type(short                 , 7)
        qor_pp_refl_register_type(int                   , 8)
        qor_pp_refl_register_type(long                  , 9)
        qor_pp_refl_register_type(long long             , 10)
        qor_pp_refl_register_type(char                  , 11)
        qor_pp_refl_register_type(wchar_t               , 12)
        qor_pp_refl_register_type(char16_t              , 13)
        qor_pp_refl_register_type(char32_t              , 14)
        qor_pp_refl_register_type(float                 , 15)
        qor_pp_refl_register_type(double                , 16)
        qor_pp_refl_register_type(long double           , 17)
        qor_pp_refl_register_type(bool                  , 18)
        qor_pp_refl_register_type(void*                 , 19)
        qor_pp_refl_register_type(const void*           , 20)
        qor_pp_refl_register_type(volatile void*        , 21)
        qor_pp_refl_register_type(const volatile void*  , 22)
        qor_pp_refl_register_type(std::nullptr_t        , 23)
        constexpr std::size_t tuple_begin_tag               = 24;
        constexpr std::size_t tuple_end_tag                 = 25;

        #undef qor_pp_refl_register_type
        ...
        constexpr std::size_t type_to_id(identity<Type*>) noexcept 
        {
            ...
            static_assert(
                ...
                "====================> QOR Reflection: Pointers to user defined types are not supported."
            );
        ...
        constexpr std::size_t type_to_id(identity<const Type*>) noexcept 
        {
            ...
            static_assert(
                std::is_same<const std::size_t, decltype(unptr)>::value,
                "====================> QOR Reflection: Const pointers to user defined types are not supported."
            );
        ...
        constexpr std::size_t type_to_id(identity<const volatile Type*>) noexcept 
        {
            ...
            static_assert(
                std::is_same<const std::size_t, decltype(unptr)>::value,
                "====================> QOR Reflection: Const volatile pointers to user defined types are not supported."
            );
        ...
        constexpr std::size_t type_to_id(identity<volatile Type*>) noexcept 
        {
            ...
            static_assert(
                std::is_same<const std::size_t, decltype(unptr)>::value,
                "====================> QOR Reflection: Volatile pointers to user defined types are not supported."
            );
        ...
        constexpr std::size_t type_to_id(identity<Type&>) noexcept 
        {
            ...
            static_assert(
                std::is_same<const std::size_t, decltype(unptr)>::value,
                "====================> QOR Reflection: References to user defined types are not supported."
            );
        ...
        {
            static_assert(!std::is_empty<Type>::value, "====================> QOR Reflection: Empty classes/structures as members are not supported.");
            return 0;
        }
        ...
        {
            static_assert(!Index, "====================> QOR Reflection: References are not supported");
            return nullptr;
        }
    ...
    constexpr void* flat_type_to_array_of_type_ids(std::size_t* types, std::index_sequence<I...>) noexcept
    {
        static_assert(
            N <= sizeof(T),
            "====================> QOR Reflection: Bit fields are not supported."
        );
    ...
    {
        static_assert(SubtupleLength == 0, "====================> QOR Reflection: Internal error while representing nested field as tuple");
    ...
    {
        static_assert(sizeof(T) == 0, "====================> QOR Reflection: Internal error while representing nested field as tuple");
        return int{};
    }
    ...
    {
        static_assert(SubtupleLength > 2, "====================> QOR Reflection: Internal error while representing nested field as tuple");
    ...
    constexpr auto internal_tuple_with_same_alignment() noexcept 
    {
        ...
        static_assert(
            ...
            "====================> QOR Reflection: Type can not be reflected without Loophole or C++17, because it's not POD"
        );
        static_assert(!std::is_reference<type>::value, "====================> QOR Reflection: Not applyable");
    ...
    struct ubiq_constructor_constexpr_copy 
    {
        ...
        constexpr operator Type() const noexcept 
        {
            static_assert(
                ...
                "====================> QOR Reflection: One of the fields in the type passed to `for_each_field` has non trivial destructor."
            );
            return {};
        }
    };
    ...
    void for_each_field_dispatcher(T& t, F&& f, std::index_sequence<I...>) 
    {
        ...
        static_assert(is_constexpr_aggregate_initializable<T, I...>::value, "====================> QOR Reflection: T must be a constexpr initializable type");
