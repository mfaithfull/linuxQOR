...
#ifndef QOR_PP_H_REFLECTION_DETAIL_FIELDSCOUNT
#define QOR_PP_H_REFLECTION_DETAIL_FIELDSCOUNT
...
namespace qor_reflection { namespace detail {
    ...
    constexpr bool static_assert_non_inherited() noexcept 
    {
        static_assert(
                ...
                "====================> QOR Reflection: QOR Reflection: Inherited types are not supported."
        );
        return true;
    }
    ...
    constexpr auto assert_first_not_base(std::index_sequence<I0, I...>) noexcept -> std::add_pointer_t<decltype(T{ ubiq_lref_base_asserting<T>{}, ubiq_lref_constructor{I}... })>
    {
        return nullptr;
    }

    template <class T, std::size_t I0, std::size_t... I, class /*Enable*/ = std::enable_if_t<!std::is_copy_constructible<T>::value>>
    constexpr auto assert_first_not_base(std::index_sequence<I0, I...>) noexcept -> std::add_pointer_t<decltype(T{ ubiq_rref_base_asserting<T>{}, ubiq_rref_constructor{I}... })>
    {
        return nullptr;
    }
    ...
    template <class T, std::size_t N>
    constexpr auto assert_first_not_base(long) noexcept -> std::enable_if_t<std::is_class<T>::value>
    {
    ...
    constexpr auto enable_if_initializable_helper(std::index_sequence<I...>) noexcept -> std::add_pointer_t<decltype(T{ubiq_lref_constructor{I}...})>;

    template <class T, std::size_t... I, class /*Enable*/ = std::enable_if_t<!std::is_copy_constructible<T>::value>>
    constexpr auto enable_if_initializable_helper(std::index_sequence<I...>) noexcept -> std::add_pointer_t<decltype(T{ubiq_rref_constructor{I}...})>;
    ...
    template <class T, std::size_t N>
    constexpr auto is_initializable(long) noexcept -> detail::enable_if_initializable_helper_t<T, N, bool>
    {
        return true;
    }
    ...
    constexpr std::size_t fields_count_binary_search(detail::one_element_range, long) noexcept 
    {
        static_assert(
            Begin == Last,
            "====================> QOR Reflection: Internal logic error."
        );
        return Begin;
    }
    ...
    template <class T, std::size_t Begin, std::size_t Last>
    constexpr auto fields_count_binary_search(detail::multi_element_range, long) noexcept -> detail::enable_if_initializable_helper_t<T, (Begin + Last + 1) / 2>
    {
    ...
    template <class T, std::size_t Begin, std::size_t N>
    constexpr auto fields_count_upper_bound(long, long) noexcept -> std::enable_if_t<(N > detail::fields_count_upper_bound_loose<T>()), std::size_t>
    {
        static_assert(
            !detail::is_initializable<T, detail::fields_count_upper_bound_loose<T>() + 1>(1L),
            "====================> QOR Reflection: Types with user specified constructors (non-aggregate initializable types) are not supported.");
        return detail::fields_count_upper_bound_loose<T>();
    }

    template <class T, std::size_t Begin, std::size_t N>
    constexpr auto fields_count_upper_bound(long, int) noexcept -> detail::enable_if_initializable_helper_t<T, N>
    {
    ...
    constexpr auto fields_count_lower_bound_unbounded(long, size_t_<0>) noexcept -> std::enable_if_t<(Begin >= detail::fields_count_upper_bound_loose<T>()), std::size_t>
    {
        static_assert(
            detail::is_initializable<T, detail::fields_count_upper_bound_loose<T>()>(1L),
            "====================> QOR Reflection: Type must be aggregate initializable.");
        return detail::fields_count_upper_bound_loose<T>();
    }
    ...
    template <class T>
    constexpr auto fields_count_dispatch(long, long, std::true_type /*are_preconditions_met*/) noexcept -> std::enable_if_t<std::is_array<T>::value, std::size_t>
    {
        return sizeof(T) / sizeof(std::remove_all_extents_t<T>);
    }

    template <class T>
    constexpr auto fields_count_dispatch(long, int, std::true_type /*are_preconditions_met*/) noexcept -> decltype(sizeof(T{}))
    {
    ...
    constexpr std::size_t fields_count() noexcept 
    {
        ...
        static_assert(
            type_is_complete,
            "====================> QOR Reflection: Type must be complete."
        );
        ...
        static_assert(
            ...
            "====================> QOR Reflection: Attempt to get fields count on a reference. This is not allowed because that could hide an issue and different library users expect different behavior in that case."
        );
        ...
        static_assert(
            ...
            "====================> QOR Reflection: Type and each field in the type must be copy constructible (or move constructible and move assignable)."
        );
        ...
        static_assert(
            ...
            "====================> QOR Reflection: Type must have no virtual function, because otherwise it is not aggregate initializable."
        );
        ...
        static_assert(
            ...
            "====================> QOR Reflection: If there's no other failed static asserts then something went wrong. Please report this issue to the github along with the structure you're reflecting."
        );
...
#endif//QOR_PP_H_REFLECTION_DETAIL_FIELDSCOUNT
