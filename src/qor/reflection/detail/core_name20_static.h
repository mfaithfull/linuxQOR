...
#ifndef QOR_PP_H_REFLECTION_DETAIL_CORENAME20_STATIC
#define QOR_PP_H_REFLECTION_DETAIL_CORENAME20_STATIC

#pragma once

#include "config.h"
#include "core.h"
#include "fake_object.h"
#include "fields_count.h"
#include "for_each_field.h"
...
namespace qor{ namespace reflection { namespace detail {
    ...
    struct core_name_skip 
    {
        ...
        consteval std::string_view apply(std::string_view sv) const noexcept 
        {
            ...
            const auto found = is_backward ? sv.rfind(until_runtime) : sv.find(until_runtime);
    ...
    #ifndef qor_pp_refl_function_signature
    #   if defined(__FUNCSIG__)
    #       define qor_pp_refl_function_signature __FUNCSIG__
    #   elif defined(__PRETTY_FUNCTION__) || defined(__GNUC__) || defined(__clang__)
    #       define qor_pp_refl_function_signature __PRETTY_FUNCTION__
    #   else
    #       define qor_pp_refl_function_signature ""
    #   endif
    #endif

        constexpr std::string_view sv = detail::clang_workaround<MsvcWorkaround>(qor_pp_refl_function_signature);
        static_assert(!sv.empty(),
            "====================> QOR Reflection: Field reflection parser configured in a wrong way. "
            "Please define the qor_pp_refl_function_signature to a compiler specific macro, "
        ...
        static_assert(
            !fn.empty(),
            "====================> QOR Reflection: Extraction of field name is misconfigured for your compiler. "
...
#   pragma clang diagnostic pop
    ...
    constexpr std::string_view get_name() noexcept 
    {
        ...
        static_assert(
            ...
            "====================> QOR Reflection: It is impossible to extract name from old C array since it doesn't have named members"
        );
        static_assert(
            sizeof(T) && qor_pp_refl_use_cpp17,
            "====================> QOR Reflection: Extraction of field's names is allowed only when the qor_pp_refl_use_cpp17 macro enabled."
    );
    ...
    {
        return qor::reflection::detail::for_each_field(
            std::forward<T>(value),
            [f = std::forward<F>(func)](auto&& field, auto index) mutable 
            {
                using IndexType = decltype(index);
                using FieldType = decltype(field);
                constexpr auto name = qor::reflection::detail::get_name<std::remove_reference_t<T>, IndexType::value>();
