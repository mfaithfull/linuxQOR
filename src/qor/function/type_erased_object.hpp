...
#ifndef QOR_PP_H_FUNCTION_TEF_TYPE_ERASED_OBJECT_IMPL
#define QOR_PP_H_FUNCTION_TEF_TYPE_ERASED_OBJECT_IMPL
...
namespace qor { namespace tef {
    ...
    {
        object_ptr_ = object.HasStoredObject() ? (*copy_constructor_)(data_, object.data_) : object.object_ptr_;
    }
    ...
    TypeErasedObject& TypeErasedObject::operator =(const TypeErasedObject& object) 
    {
        if (this == &object)
        {
            return *this;
        ...
        copy_constructor_ = object.copy_constructor_;
        move_constructor_ = object.move_constructor_;
        object_ptr_ = HasStoredObject() ? (*copy_constructor_)(data_, object.data_) : object.object_ptr_;
        return *this;
    }
    ...
    void TypeErasedObject::StoreObject(T&& object) 
    {
        ...
        static_assert(sizeof(data_) >= sizeof(CustomUniquePtr<U>), "Buffer is too small.");

        static_assert(alignof(DataBuffer<void>) >= alignof(CustomUniquePtr<U>), "Incompatible unique_ptr type alignments.");
        ...
        if (allocator.first) 
        {
            ...
            check(heap, Error::kCustomAllocator);
    ...
    void TypeErasedObject::StoreObject(const std::shared_ptr<T>& object) 
    {
        ...
        static_assert(sizeof(data_) >= sizeof(std::shared_ptr<T>), "Buffer is too small.");

        static_assert(alignof(DataBuffer<void>) >= alignof(std::shared_ptr<T>), "Incompatbile std::shared_ptr type alignments.");
    ...
    template <typename T>
    void* TypeErasedObject::CopySharedPointer(void* dest, const void* src) 
    {
        static_assert(IsSharedPtr<T>::value, "Type is not a shared_ptr.");
        auto src_obj = reinterpret_cast<const T*>(src);
        debug_check(dest, Error::kInvalidObject);
        debug_check(src_obj, Error::kInvalidObject);
    ...
    TypeErasedObject::CopyHeapObject(void*, const void*) 
    {
        ...
        serious(tefError.at(Error::kNonCopyableObject));
        return nullptr;
    }
    ...
    {
        static_assert(IsUniquePtr<T>::value || IsSharedPtr<T>::value, "Type is not a unique_ptr or shared_ptr.");
    ...
    void TypeErasedObject::DestroyObject(void* obj_erased) 
    {
        ...
        debug_check(obj, Error::kInvalidObject);
        obj->~T();
    }
    ...
    void TypeErasedObject::CustomAllocatorDeleter<T>::operator ()(T* ptr) 
    {
        ...
        if (deallocator.first) 
        {
            ...
            {
                check(false, Error::kCustomAllocator);
            }
        }
        else 
        {
...
#endif//QOR_PP_H_FUNCTION_TEF_TYPE_ERASED_OBJECT_IMPL
