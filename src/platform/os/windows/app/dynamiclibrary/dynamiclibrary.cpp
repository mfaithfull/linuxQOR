...
namespace qor{ namespace framework{ namespace win{
    ...
    {
        m_handle = reinterpret_cast<void*>(qor::win::api::Kernel32::LoadLibrary(qor::platform::win::to_tstring(libName.c_str()).c_str()));
    }
    ...
    DynamicLibrary::~DynamicLibrary()
    {
        if(m_handle)
        {
            qor::win::api::Kernel32::FreeLibrary((HMODULE)m_handle);
            m_handle = nullptr;
        }
    }
    ...
    DynamicLibrary::DynProc DynamicLibrary::GetProcAddress(const std::string& procName)
    {
        ...
        if(m_handle != nullptr)
        {
            result = reinterpret_cast<DynamicLibrary::DynProc>(
                qor::win::api::Kernel32::GetProcAddress(
