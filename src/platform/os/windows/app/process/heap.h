...
#ifndef QOR_PP_H_OS_WINDOWS_APP_PROCESS_HEAP
#define QOR_PP_H_OS_WINDOWS_APP_PROCESS_HEAP

#include "src/platform/os/windows/common/handles/handle.h"

namespace qor{ namespace win{

    class qor_pp_module_interface(QOR_WINDOWSPROCESS) ProcessHeap
    {
    public:
        ...
        ProcessHeap();
        virtual ~ProcessHeap() noexcept;
       
		byte* Alloc(unsigned long flags, size_t bytes);
		size_t Compact(unsigned long flags);
		bool Free(unsigned long flags, byte* mem);
		bool Lock();
    ...
    private:
        ...
        platform::win::PrimitiveHandle m_handle;
    };
