...
namespace qor { namespace io { namespace async { namespace win {
	...
	bool WriteOperationImpl::try_start(win32_overlapped_operation_base& operation) noexcept
	{
        const DWORD numberOfBytesToWrite = m_buffer.len <= 0xFFFFFFFF ? static_cast<DWORD>(m_buffer.len) : DWORD(0xFFFFFFFF);

		DWORD numberOfBytesWritten = 0;
        BOOL ok = qor::win::api::Kernel32::WriteFile(
            m_file->m_handle,
            m_buffer.buf,
	...
	void WriteOperationImpl::cancel(win32_overlapped_operation_base& operation) noexcept
	{
