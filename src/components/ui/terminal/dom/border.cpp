...
namespace qor{ namespace components{ namespace tui {
    ...
    namespace 
    {
        ...
        {
        public:
            Border(Elements children, BorderStyle style, std::optional<Colour> foreground_color = std::nullopt) : Node(std::move(children)), charset_(simple_border_charset[style]), foreground_color_(foreground_color) {} 

            const Charset& charset_;
            std::optional<Colour> foreground_color_;
            ...
            void Render(Screen& screen) override 
            {
                ...
                screen.at(box_.x_min, box_.y_min) = charset_[0];
                screen.at(box_.x_max, box_.y_min) = charset_[1];
                screen.at(box_.x_min, box_.y_max) = charset_[2];
                screen.at(box_.x_max, box_.y_max) = charset_[3];

                for (int x = box_.x_min + 1; x < box_.x_max; ++x) 
                {
                    Pixel& p1 = screen.PixelAt(x, box_.y_min);
                    Pixel& p2 = screen.PixelAt(x, box_.y_max);
                    p1.character = charset_[4];
                    p2.character = charset_[4];
                    p1.automerge = true;
                    p2.automerge = true;
                }
                for (int y = box_.y_min + 1; y < box_.y_max; ++y) 
                {
                    ...
                    p3.character = charset_[5];
                    p4.character = charset_[5];
                    p3.automerge = true;
                    p4.automerge = true;
                }
    ...
    Decorator borderStyled(Colour foreground_color) 
    {
        return [foreground_color](Element child) 
        {
            return std::make_shared<Border>(unpack(std::move(child)), ROUNDED, foreground_color);
        };
    }

    //Same as border but with a foreground color and a different style
    Decorator borderStyled(BorderStyle style, Colour foreground_color) 
    {
        return [style, foreground_color](Element child) 
        {
            return std::make_shared<Border>(unpack(std::move(child)), style, foreground_color);
        };
    }
    ...
    {
        return std::make_shared<Border>(unpack(std::move(content), std::move(title)), border);
    }
