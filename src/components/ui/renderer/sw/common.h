...
#ifndef QOR_PP_H_COMPONENTS_FRAMEWORK_UI_RENDERER_SW_COMMON
#define QOR_PP_H_COMPONENTS_FRAMEWORK_UI_RENDERER_SW_COMMON
...
#define SW_COLOR_TABLE 1024
    ...
    struct SwRle
    {
        ...
        const SwSpan* fetch(int32_t min, uint32_t max, const SwSpan** end) const
        {
            ...
            if (min <= spans.first().y) {
                ...
                begin = std::lower_bound(spans.begin(), spans.end(), min, comp);
            }
            if (end) {
                if (max >= spans.last().y) {
                    *end = spans.end();
                    ...
                    *end = std::upper_bound(spans.begin(), spans.end(), max, comp);
                }
            }
            return begin;
        }
    ...
    struct SwCellPool
    {
        ...
        SwCellPool() : size(DEFAULT_POOL_SIZE), buffer(malloc<SwCell>(DEFAULT_POOL_SIZE)) {}
    ...
    static inline uint32_t opBlendExclusion(uint32_t s, uint32_t d)
    {
        ...
        auto f = [](uint8_t s, uint8_t d) {
            return clamp(s + d - 2 * MULTIPLY(s, d), 0, 255);
        };
...
#endif//QOR_PP_H_COMPONENTS_FRAMEWORK_UI_RENDERER_SW_COMMON