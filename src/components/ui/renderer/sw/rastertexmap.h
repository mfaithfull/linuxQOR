...
#ifndef QOR_PP_H_COMPONENTS_FRAMEWORK_UI_RENDERER_SW_RASTERTEXMAP
#define QOR_PP_H_COMPONENTS_FRAMEWORK_UI_RENDERER_SW_RASTERTEXMAP
...
namespace qor{ namespace components{ namespace ui{ namespace renderer{
    ...
    static void _rasterPolygonImage(SwSurface* surface, const SwImage& image, const RenderRegion& bbox, Polygon& polygon, AASpans* aaSpans, uint8_t opacity)
    {
        ...
        if (equal(y[0], y[1])) side = x[0] > x[1];
        if (equal(y[1], y[2])) side = x[2] > x[1];
    ...
    {
        auto aaSpans = malloc<AASpans>(sizeof(AASpans));
        ...
        aaSpans->lines = malloc<AALine>(height * sizeof(AALine));
...
#endif//QOR_PP_H_COMPONENTS_FRAMEWORK_UI_RENDERER_SW_RASTERTEXMAP