...
namespace qor{ namespace components{ namespace ui{ namespace renderer{
    ...
    {
        if (!params->rd) params->rd = malloc<SwGaussianBlur>(sizeof(SwGaussianBlur));
    ...
    {
        if (!params->rd) params->rd = malloc<SwDropShadow>(sizeof(SwDropShadow));
        ...
        //offset
        auto radians = deg2rad(90.0f - params->angle) - radian(transform);
        rd->offset = {(int32_t)((params->distance * scale) * cosf(radians)), (int32_t)(-1.0f * (params->distance * scale) * sinf(radians))};
