...
namespace qor { namespace data { namespace parser { namespace json {
    ...
    escapeSequence::escapeSequence(Parser* parser) : Sequence(parser, 
            ...
            new_ref<AnyOneOf>(parser, 
                new_ref<Specific<uint32_t>>(parser, 0x22, static_cast<uint64_t>(jsonToken::quotation_mark)),
                new_ref<AnyOneOf>(parser,
                    new_ref<Specific<uint32_t>>(parser, 0x5C),// '\\'
                    new_ref<AnyOneOf>(parser,
                        new_ref<Specific<uint32_t>>(parser, 0x2F),//  '/'
                        new_ref<AnyOneOf>(parser,
                            new_ref<Specific<uint32_t>>(parser, 0x62),// 'b'
                            new_ref<AnyOneOf>(parser,
                                new_ref<Specific<uint32_t>>(parser, 0x66),// 'f'
                                new_ref<AnyOneOf>(parser,
                                    new_ref<Specific<uint32_t>>(parser, 0x6E),// 'n'
                                    new_ref<AnyOneOf>(parser,
                                        new_ref<Specific<uint32_t>>(parser, 0x72),// 'r'
                                        new_ref<AnyOneOf>(parser,
                                            new_ref<Specific<uint32_t>>(parser, 0x74),// 't'
