...
namespace qor{ namespace components{ 
    ...
    bool BaseDeserializerSink::Write(size_t& unitsWritten, size_t unitsToWrite)
    {
        return (unitsToWrite == 0 || Pull(unitsWritten, unitsToWrite)) ? Push(unitsWritten, unitsWritten) : false;
    }

    bool BaseDeserializerSink::Pull(size_t& unitsWritten, size_t unitsToWrite)
    {
    ...
    bool BaseDeserializerSink::Push(size_t& unitsWritten, size_t unitsToWrite)
    {
        if(unitsToWrite > 0)
        {
            ...
            if(buffer)
            {
                ...
                size_t bytesParsed = Read(data, buffer->GetUnitSize() * unitsToWrite);
                if(bytesParsed > 0)
                {
