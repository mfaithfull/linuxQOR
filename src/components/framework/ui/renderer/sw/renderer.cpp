...
namespace qor{ namespace components{ namespace ui{ namespace renderer{
    ...
    static std::atomic<int32_t> rendererCnt{-1};
    ...
    struct SwShapeTask : SwTask
    {
        ...
        {
            if (!rshape->stroke || zero(rshape->stroke->width)) return 0.0f;
            if (!clipper && (!rshape->stroke->fill && (rshape->stroke->color.a == 0))) return 0.0f;
            if (zero(rshape->stroke->trim.begin - rshape->stroke->trim.end)) return 0.0f;
    ...
    SwSurface* SwRenderer::request(int channelSize, bool square)
    {
        ...
        if (!cmp) {
            ...
            cmp->compositor->image.data = malloc<pixel_t>(channelSize * w * h);
    ...
    bool SwRenderer::intersectsImage(RenderData data, const RenderRegion& region)
    {
        ...
        auto rad = radian(task->transform);
