...
class qor_pp_module_interface(QOR_TUI) MenuBase : public ComponentBase, public MenuOption 
{
 public:
  ...
  bool IsHorizontal() { return qor::components::tui::IsHorizontal(direction); }
  void OnChange() {
    if (on_change) {
      on_change();
    }
  ...
  Decorator AnimatedColorStyle(int i) {
    ...
    if (entries_option.animated_colors.foreground.enabled) {
      style = style | color(Colour::Interpolate(
    ...
    if (entries_option.animated_colors.background.enabled) {
      style = style | bgcolor(Colour::Interpolate(
...
///   entry 3
/// ```
qor_pp_module_interface(QOR_TUI) Component Menu(MenuOption option) {
...
///   entry 3
/// ```
qor_pp_module_interface(QOR_TUI) Component Menu(ConstStringListRef entries, int* selected, MenuOption option) {
...
///   entry 3
/// ```
qor_pp_module_interface(QOR_TUI) Component MenuEntry(ConstStringRef label, MenuEntryOption option) {
...
///   entry 3
/// ```
qor_pp_module_interface(QOR_TUI) Component MenuEntry(MenuEntryOption option) {
   ...
   private:
    ...
    Decorator AnimatedColorStyle() {
      ...
      if (animated_colors.foreground.enabled) {
        style = style |
                color(Colour::Interpolate(animation_foreground_,
      ...
      if (animated_colors.background.enabled) {
        style = style |
                bgcolor(Colour::Interpolate(animation_background_,
