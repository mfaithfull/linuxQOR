...
namespace qor { namespace components { namespace protocols { namespace http {
    ...
    void field_vchar::Prepare()
    {
        //std::cout << "Looking for a field_vchar." << std::endl;
        GetParser()->PushNode(new_ref<FieldVCharNode>());
    }

    void field_vchar::Emit()
    {
        //std::cout << "Emitting a field_vchar." << std::endl;
        char c = '\0';
        ...
        if(node.IsNotNull() && node->GetToken() != m_token)
        {
            ...
            if(token == static_cast<uint64_t>(parser::eToken::Char)
                || token == static_cast<uint64_t>(parser::eToken::VisibleChar))
            {
        ...
        if(node.IsNotNull())
        {
            ...
            charNode->GetObject()->m_value = c;
            GetParser()->PushNode(node);
        }        
    }

    void field_vchar::Fail()
    {
        auto node = GetParser()->PopNode();
        if(node.IsNotNull() && node->GetToken() != m_token)
        {
