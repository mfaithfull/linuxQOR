...
namespace qor { namespace components { namespace protocols { namespace http {
    ...
    {
        //std::cout << "Looking for a uri_origin_form." << std::endl;
        GetParser()->PushNode(new_ref<UriOriginFormNode>());
    }

    void uri_origin_form::Emit()
    {
        //std::cout << "Emitting a uri_origin_form." << std::endl;
        ...
        while(node.IsNotNull() && node->GetToken() != m_token)
        {
            ...
            {
                auto pathNode = node.AsRef<UriAbsolutePathNode>();
                if(pathNode.IsNotNull())
                {
                    uriPath = pathNode->GetObject()()();
                }
            }
            else if(token == static_cast<uint64_t>(httpRequestToken::query))
            {
                auto queryNode = node.AsRef<QueryNode>();
                if(queryNode.IsNotNull())
                {
                    query = queryNode->GetObject()()();
                }
            }
            else
            {
        ...
        if(node.IsNotNull())
        {
            auto uriNode = node.AsRef<UriOriginFormNode>();
            if(uriNode.IsNotNull())
            {
                uriNode->GetObject()->m_path = uriPath;
                uriNode->GetObject()->m_query = query;
                std::cout << "Uri Origin." << std::endl;
            }
            GetParser()->PushNode(node);
        }        
    }

    void uri_origin_form::Fail()
    {
        auto node = GetParser()->PopNode();
