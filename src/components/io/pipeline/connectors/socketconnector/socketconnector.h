...
namespace qor{ namespace io{ namespace network { namespace components{
    ...
    class qor_pp_module_interface(QOR_SOCKETCONNECTOR) SocketConnector : public pipeline::Plug, virtual public qor::io::network::ISocket
    {
    public:
        ...
        void Configure(const std::string &host, int port, const std::string &ip = "", qor::io::network::sockets::eAddressFamily address_family = qor::io::network::sockets::eAddressFamily::AF_INet, qor::io::network::addrinfo_flags socket_flags = 0, bool tcp_nodelay = false, bool ipv6_v6only = false, time_t timeout_sec = 0);
        const qor::io::network::Address& RemoteAddress();
        void SetRemoteAddress(qor::io::network::Address& address);
        bool ConnectToAddress(const std::string &host, int port, const std::string &ip, qor::io::network::sockets::eAddressFamily address_family, qor::io::network::addrinfo_flags socket_flags, bool tcp_nodelay, bool ipv6_v6only, time_t timeout_sec = 0 );
        bool SetNonBlocking(bool nonBlocking);

        //ISocket implementation over owned socket instance

        virtual int32_t Bind(const qor::io::network::Address& Address);
        ...
        virtual ref_of<qor::io::network::Socket>::type Accept(qor::io::network::Address& Address);
        ...
        virtual int32_t Connect(const qor::io::network::Address& Address);
        virtual int32_t GetPeerName(qor::io::network::Address& Address);
        virtual int32_t GetSockName(qor::io::network::Address& Address);
        ...
        virtual int32_t ReceiveFrom(char* Buffer, int32_t iLen, int32_t iFlags, qor::io::network::Address& From);
        ...
        virtual int32_t SendTo(const char* Buffer, int32_t iLen, int32_t iFlags, const qor::io::network::Address& To);
        virtual int32_t Shutdown(qor::io::network::sockets::eShutdown how);
    ...
    private:
        ...
        void SetupHintsForGetAddressInfo(qor::io::network::AddressInfo& hints, const std::string &host, const std::string &ip, qor::io::network::sockets::eAddressFamily address_family, qor::io::network::addrinfo_flags socket_flags, std::string& node);
        bool AttemptConnections(ref_of<qor::io::network::Sockets>::type socketsSubsystem, std::vector<qor::io::network::AddressInfo>& addresses);
    };
