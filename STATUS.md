29/07/2026: 
The UI design is still in progress but things are coallescing slowly around a reasonable compormise.
The parser has had a lot of work and now reads most Unicode text encodings. The JSON parser needs upgrading to 
be layered over Unicode text before we can use it to load UI scene templates.
Currently trying out ways to add Pipes in a platform agnostic form via sockets on Linux and Files on Windows.
Planning to post to r/cpp Show and Tell in a couple of days.
