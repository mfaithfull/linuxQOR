...
#include <algorithm>
using namespace qor;
struct FrozenUnorderedMapTestSuite{};

qor_pp_test_suite_case(FrozenUnorderedMapTestSuite,SingletonFrozenUnorderedMap) 
{
    ...
    qor_pp_assert_that(find == 2.);

    auto notfound = ze_map.find(3);
    qor_pp_assert_that(notfound == ze_map.end());

    auto found = ze_map.find(1);
    qor_pp_assert_that(found == ze_map.begin());

    auto range = ze_map.equal_range(1);
    qor_pp_assert_that(std::get<0>(range) == ze_map.begin());
...
qor_pp_test_suite_case(FrozenUnorderedMapTestSuite,FrozenUnorderedMapPowersOfTwo) 
{
    ...
    {
        qor_pp_assert_that(pair.first == (1 << pair.second));
    }
}
...
qor_pp_test_suite_case(FrozenUnorderedMapTestSuite, FrozenUnorderedMapMultiplesOf64) 
{
...
# undef M64
    ...
    for (const auto & pair : frozen_map) 
    {
        qor_pp_assert_that(pair.first == 64 * pair.second);
    }
}

qor_pp_test_suite_case(FrozenUnorderedMapTestSuite, FrozenUnorderedMap_stdUnorderedMap) 
{
    #define INIT_SEQ                                                               \
    ...
    static_assert(std::is_same<typename decltype(std_map)::key_type, typename decltype(frozen_map)::key_type>::value, "");
    static_assert(std::is_same<typename decltype(std_map)::mapped_type, typename decltype(frozen_map)::mapped_type>::value, "");
}

qor_pp_test_suite_case(FrozenUnorderedMapTestSuite, FrozenUnorderedMap_MakeUnorderedMap) 
{
    constexpr frozen::unordered_map<int, int, 128> frozen_map = { INIT_SEQ };
...
qor_pp_test_suite_case(FrozenUnorderedMapTestSuite, FrozenUnorderedMap_stdUnorderedMa_small) 
{
...
qor_pp_test_suite_case(FrozenUnorderedMapTestSuite, FrozenUnorderedMap_Constexpr) 
{
...
qor_pp_test_suite_case(FrozenUnorderedMapTestSuite, Access) 
{
    constexpr frozen::unordered_map<unsigned, unsigned, 2> ce = {{3,4}, {11,12}};
    qor_pp_assert_that(4 == ce.at(3));
    qor_pp_test_assert_throw(ce.at(33), std::exception);    
}

qor_pp_test_suite_case(FrozenUnorderedMapTestSuite, ModifiableFrozenUnorderedMap) 
{
...
qor_pp_test_suite_case(FrozenUnorderedMapTestSuite, FrozenUnorderedMapHeterogeneousContainer) 
{
    constexpr auto map = frozen::make_unordered_map<frozen::string, int>(
            {{"one", 1}, {"two", 2}, {"three", 3}},
            frozen::elsa<>{}, eq{});

    qor_pp_assert_that(map.find(std::string{"two"})->second == 2);
    qor_pp_assert_that(map.find(frozen::string{"two"})->second == 2);
}
...
qor_pp_test_suite_case(FrozenUnorderedMapTestSuite, FrozenUnorderedMapTransparentContainer) 
{
    constexpr frozen::unordered_map<frozen::string, int, 1, frozen::anna<>, std::equal_to<>> map = {{"one", 1}};

    qor_pp_assert_that(map.find(case_insensitive("OnE"))->second == 1);
    qor_pp_assert_that(map.count(case_insensitive("One")) == 1);
    qor_pp_assert_that(map.count(case_insensitive("TwO")) == 0);
    qor_pp_assert_that(map.count(case_insensitive("333")) == 0);
}
