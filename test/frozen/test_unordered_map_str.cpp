...
using namespace qor;
struct FrozenUnorderedMapStrTestSuite{};

qor_pp_test_suite_case(FrozenUnorderedMapStrTestSuite,FrozenUnorderedMapStrInt) 
{
    #define INIT_SEQ                                                               \
    ...
    qor_pp_assert_that(std_map.size() == frozen_map.size());
    for (auto v : std_map)
        qor_pp_assert_that(frozen_map.count(v.first));

    for (auto v : frozen_map)
        qor_pp_assert_that(std_map.count(v.first));
    }

}

qor_pp_test_suite_case(FrozenUnorderedMapStrTestSuite,VariousFrozenUnorderedMapConfig) 
{
