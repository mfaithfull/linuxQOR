...
using namespace qor;
struct TestFrozenMap{};

qor_pp_test_suite_case(TestFrozenMap, EmptyFrozenMap) 
{
    ...
    qor_pp_assert_that(find == ze_map.end());

    constexpr auto range = ze_map.equal_range(0);
    qor_pp_assert_that(std::get<0>(range) == ze_map.end());
    qor_pp_assert_that(std::get<1>(range) == ze_map.end());

    constexpr auto lower_bound = ze_map.lower_bound(1);
    qor_pp_assert_that(lower_bound == ze_map.end());

    constexpr auto upper_bound = ze_map.upper_bound(1);
    qor_pp_assert_that(upper_bound == ze_map.end());
    ...
    qor_pp_assert_that(std::distance(ze_map.rbegin(), ze_map.rend()) == 0);
    qor_pp_assert_that(std::count(ze_map.crbegin(), ze_map.crend(),
                        decltype(ze_map)::value_type(3, 5.3f)) == 0);
}

qor_pp_test_suite_case(TestFrozenMap, SingletonFrozenMap) 
{
    ...
    qor_pp_assert_that(find1 == ze_map.begin());
    qor_pp_assert_that(std::get<0>(*find1) == 1);
    qor_pp_assert_that(std::get<1>(*find1) == 3.14);

    const auto find5 = ze_map.find(5);
    qor_pp_assert_that(find5 == ze_map.end());

    const auto range0 = ze_map.equal_range(0);
    qor_pp_assert_that(std::get<0>(range0) == ze_map.begin());
    qor_pp_assert_that(std::get<1>(range0) == ze_map.begin());

    const auto range1 = ze_map.equal_range(1);
    qor_pp_assert_that(std::get<0>(range1) == ze_map.begin());
    qor_pp_assert_that(std::get<1>(range1) == ze_map.end());

    const auto range2 = ze_map.equal_range(2);
    qor_pp_assert_that(std::get<0>(range2) == ze_map.end());
    qor_pp_assert_that(std::get<1>(range2) == ze_map.end());

    const auto lower_bound0 = ze_map.lower_bound(0);
    qor_pp_assert_that(lower_bound0 == ze_map.begin());

    const auto lower_bound1 = ze_map.lower_bound(1);
    qor_pp_assert_that(lower_bound0 == ze_map.begin());
    qor_pp_assert_that(lower_bound1 == ze_map.find(1));

    const auto lower_bound2 = ze_map.lower_bound(2);
    qor_pp_assert_that(lower_bound2 == ze_map.end());

    const auto upper_bound0 = ze_map.upper_bound(0);
    qor_pp_assert_that(upper_bound0 == ze_map.begin());

    const auto upper_bound1 = ze_map.upper_bound(1);
    qor_pp_assert_that(upper_bound1 == ze_map.end());

    const auto upper_bound2 = ze_map.upper_bound(2);
    qor_pp_assert_that(upper_bound2 == ze_map.end());
    ...
    qor_pp_assert_that(std::distance(ze_map.rbegin(), ze_map.rend()) == 1);
    qor_pp_assert_that(std::count(ze_map.crbegin(), ze_map.crend(),
...
qor_pp_test_suite_case(TestFrozenMap, TripleFrozenMap) 
{
    ...
    qor_pp_assert_that(find10 == ze_map.begin());
    qor_pp_assert_that(std::get<0>(*find10) == 10);
    qor_pp_assert_that(std::get<1>(*find10) == true);

    const auto find15 = ze_map.find(15);
    qor_pp_assert_that(find15 == ze_map.end());
    ...
    qor_pp_assert_that(std::get<0>(range1) == ze_map.begin());
    qor_pp_assert_that(std::get<1>(range1) == ze_map.begin() + 1);

    const auto lower_bound0 = ze_map.lower_bound(0);
    qor_pp_assert_that(lower_bound0 == ze_map.begin());

    const auto lower_bound1 = ze_map.lower_bound(20);
    qor_pp_assert_that(lower_bound1 == ze_map.begin() + 1);

    const auto lower_bound2 = ze_map.lower_bound(25);
    qor_pp_assert_that(lower_bound2 == ze_map.begin() + 2);

    const auto lower_bound3 = ze_map.lower_bound(40);
    qor_pp_assert_that(lower_bound3 == ze_map.end());

    for (auto val : ze_map) 
    {
        const auto lower_bound = ze_map.lower_bound(std::get<0>(val));
        qor_pp_assert_that(lower_bound == ze_map.find(std::get<0>(val)));
    }
    ...
    qor_pp_assert_that(upper_bound1 == (ze_map.begin() + 1));

    const auto upper_bound2 = ze_map.upper_bound(15);
    qor_pp_assert_that(upper_bound2 == ze_map.begin() + 1);

    const auto upper_bound3 = ze_map.upper_bound(40);
    qor_pp_assert_that(upper_bound3 == ze_map.end());

    auto const begin = ze_map.begin(), end = ze_map.end();
    qor_pp_assert_that((begin + ze_map.size()) == end);
    ...
    auto const cbegin = ze_map.cbegin(), cend = ze_map.cend();
    qor_pp_assert_that(cbegin == (cend - ze_map.size()));

    std::for_each(ze_map.begin(), ze_map.end(), [](std::tuple<unsigned long, bool>) {});
    qor_pp_assert_that((std::size_t)std::distance(ze_map.rbegin(), ze_map.rend()) == ze_map.size());
    qor_pp_assert_that(std::count(ze_map.crbegin(), ze_map.crend(),
                        decltype(ze_map)::value_type(3, 14)) == 0);
    qor_pp_assert_that(std::count(ze_map.crbegin(), ze_map.crend(),
...
qor_pp_test_suite_case(TestFrozenMap, FrozenMap_StdMap) 
{
#define INIT_SEQ                                                               \
    ...
    {
        qor_pp_assert_that(std_map.size() == frozen_map.size());
        for (auto v : std_map) {
            qor_pp_assert_that(frozen_map.count(std::get<0>(v)));
            qor_pp_assert_that(frozen_map.find(std::get<0>(v))->second == v.second);
        }
        for (auto v : frozen_map)
            qor_pp_assert_that(std_map.count(std::get<0>(v)));
    }
...
qor_pp_test_suite_case(TestFrozenMap, FrozenMap_FrozenMake_map) 
{
    constexpr frozen::map<int, int, 128> frozen_map = { INIT_SEQ };
    ...
    qor_pp_assert_that(std::equal(frozen_map2.begin(), frozen_map2.end(), frozen_map3.begin()));

    //SECTION("checking size and content") 
    {
        qor_pp_assert_that(frozen_map.size() == frozen_map2.size());
        for (auto v : frozen_map2)
            qor_pp_assert_that(frozen_map.count(std::get<0>(v)));
        for (auto v : frozen_map)
            qor_pp_assert_that(frozen_map2.count(std::get<0>(v)));
    }
    ...
    {
        qor_pp_assert_that(frozen_empty_map.empty());
        qor_pp_assert_that(frozen_empty_map.size() == 0);
        qor_pp_assert_that(frozen_empty_map.begin() == frozen_empty_map.end());

        qor_pp_assert_that(frozen_empty_map2.empty());
        qor_pp_assert_that(frozen_empty_map2.size() == 0);
        qor_pp_assert_that(frozen_empty_map2.begin() == frozen_empty_map2.end());

        qor_pp_assert_that(frozen_empty_map3.empty());
        qor_pp_assert_that(frozen_empty_map3.size() == 0);
        qor_pp_assert_that(frozen_empty_map3.begin() == frozen_empty_map3.end());
    }
}

qor_pp_test_suite_case(TestFrozenMap, FrozenMapConstexpr) 
{
...
qor_pp_test_suite_case(TestFrozenMap, ModifiableFrozenMap) 
{
    frozen::map<int, int, 3> frozen_map = {{0, 1}, {2, 3}, {4, 5}};

    //SECTION("Lookup existing values") 
    {
        qor_pp_assert_that(frozen_map.at(0) == 1);
        qor_pp_assert_that(frozen_map.find(0)->second == 1);
        qor_pp_assert_that(frozen_map.equal_range(0).first->second == 1);

        qor_pp_assert_that(frozen_map.at(2) == 3);
        qor_pp_assert_that(frozen_map.find(2)->second == 3);
        qor_pp_assert_that(frozen_map.equal_range(2).first->second == 3);

        qor_pp_assert_that(frozen_map.at(4) == 5);
        qor_pp_assert_that(frozen_map.find(4)->second == 5);
        qor_pp_assert_that(frozen_map.equal_range(4).first->second == 5);
    }

    //SECTION("Lookup failure") 
    {
        qor_pp_assert_that(frozen_map.find(3) == frozen_map.end());
        qor_pp_test_assert_throw(frozen_map.at(3), std::out_of_range);        

        qor_pp_assert_that(frozen_map.find(5) == frozen_map.end());
        qor_pp_test_assert_throw(frozen_map.at(5), std::out_of_range);
    }
    ...
    //SECTION("Modify value") 
    {
        ...
        qor_pp_assert_that(frozen_map.begin()->second == -2);

        (frozen_map.end() - 1)->second = -3;
        qor_pp_assert_that((frozen_map.end() - 1)->second == -3);

        frozen_map.equal_range(4).first->second = -5;
        qor_pp_assert_that(frozen_map.at(4) == -5);

        frozen_map.lower_bound(2)->second = -22;
        qor_pp_assert_that(frozen_map.at(2) == -22);

        frozen_map.upper_bound(2)->second = -33;
        qor_pp_assert_that(frozen_map.at(4) == -33);
    }
}
...
qor_pp_test_suite_case(TestFrozenMap, EmptyFrozenTransparentMap) 
{
...
qor_pp_test_suite_case(TestFrozenMap, FrozenTransparentMap) 
{
    ...
    constexpr auto size = ze_map.size();
    qor_pp_assert_that(size == 4);

    constexpr auto max_size = ze_map.max_size();
    qor_pp_assert_that(max_size == 4);
    ...
    const auto find10 = ze_map.find(10);
    qor_pp_assert_that(find10 == ze_map.begin());
    qor_pp_assert_that(std::get<0>(*find10) == S{10});
    qor_pp_assert_that(std::get<1>(*find10) == true);
...
qor_pp_test_suite_case(TestFrozenMap, FrozenMap_FrozenMakeMapTransparent) 
{
...
qor_pp_test_suite_case(TestFrozenMap, FrozenMapConstexprStdPairKey) 
{
