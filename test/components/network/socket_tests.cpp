...
#include <cppcoro/AsyncScope.hpp>
		...
		auto receive = [&]() -> task<int>
		{
			...
			do
			{
				...
				for (std::size_t i = 0; i < bytesReceived; ++i)
				{
					...
					qor_pp_assert_that(buffer[i] == expectedByte);
				}

				totalBytesReceived += bytesReceived;
			} while (bytesReceived > 0);

			qor_pp_assert_that(totalBytesReceived == 1000);
...
TEST_CASE("send/recv TCP/IPv4 many connections")
{
	...
	auto echoServer = [&](CancellationToken ct) -> task<>
	{
...
TEST_CASE("udp send_to/recv_from")
{
	...
	auto server = [&](socket serverSocket) -> task<int>
	{
		...
		qor_pp_assert_that(bytesReceived == 50);
	...
	auto client = [&]() -> task<int>
	{
		...
		// Receive ACK message
		{
			...
			qor_pp_assert_that(bytesReceived == 1);
			qor_pp_assert_that(buffer[0] == 0);
			qor_pp_assert_that(ackAddress == serverAddress);
		}
