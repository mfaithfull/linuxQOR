...
TEST_CASE("to_string" * doctest::skip{ isMsvc15_5X86Optimised })
{
	...
	qor_pp_assert_that(a.to_string() == "192.168.2.254:80");
	qor_pp_assert_that(b.to_string() == "[2001:db8:85a3::8a2e:370:7334]:22");
}

TEST_CASE("from_string" * doctest::skip{ isMsvc15_5X86Optimised })
{
	qor_pp_assert_that(ip_endpoint::from_string("") == std::nullopt);
	qor_pp_assert_that(ip_endpoint::from_string("[foo]:123") == std::nullopt);
	qor_pp_assert_that(ip_endpoint::from_string("[123]:1000") == std::nullopt);
	qor_pp_assert_that(ip_endpoint::from_string("[10.11.12.13]:1000") == std::nullopt);

	qor_pp_assert_that(ip_endpoint::from_string("192.168.2.254:80") ==
		ipv4_endpoint{
			ipv4_address{ 192, 168, 2, 254 }, 80 });
	qor_pp_assert_that(ip_endpoint::from_string("[2001:0db8:85a3:0000:0000:8a2e:0370:7334]:443") ==
		ipv6_endpoint{
