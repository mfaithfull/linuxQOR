...
TEST_CASE("construct/destruct")
{
	...
	qor_pp_assert_that(threadPool.thread_count() == std::thread::hardware_concurrency());
}

TEST_CASE("construct/destruct to specific thread count")
{
	static_thread_pool threadPool{ 5 };
	qor_pp_assert_that(threadPool.thread_count() == 5);
}

TEST_CASE("run one task")
{
	static_thread_pool threadPool{ 2 };
...
task<std::uint64_t> sum_of_squares(
	std::uint32_t start,
	std::uint32_t end,
...
TEST_CASE("launch sub-task with many sub-tasks")
{
	...
	auto result = sync_wait(sum_of_squares(0, limit , tp));
...
task<void> for_each_async(SCHEDULER& scheduler, RANGE& range, FUNC func)
{
...
TEST_CASE("for_each_async")
{
...

	{
		...
		sync_wait([&]() -> task<>
		{
			...
			{
				qor_pp_assert_that(values[i] == collatz_distance(i + 1));
			}
		}());
	}
