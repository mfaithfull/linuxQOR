...
TEST_CASE_FIXTURE(temp_dir_fixture, "write a file")
{
	...
	auto write = [&](io_service& ioService) -> task<>
	{
		std::printf(" starting write\n"); std::fflush(stdout);

		auto f = write_only_file::open(ioService, filePath);

		qor_pp_assert_that(f.size() == 0);
	...
	auto read = [&](io_service& io) -> task<>
	{
		std::printf(" starting read\n"); std::fflush(stdout);

		auto f = read_only_file::open(io, filePath);

		const auto fileSize = f.size();

		qor_pp_assert_that(fileSize == 10240);
		...
		for (std::uint64_t i = 0; i < fileSize;)
		{
			...
			{
				qor_pp_assert_that(buffer[j] == ('a' + ((i % 1024) % 26)));
			}
		}
	};
	...
	sync_wait(when_all(
		[&]() -> task<int>
		{
			auto stopOnExit = on_scope_exit([&] { ioService.stop(); });
...
TEST_CASE_FIXTURE(temp_dir_with_io_service_fixture, "read write file")
{
	auto run = [&]() -> task<>
	{
		io_work_scope ioScope{ io_service() };
		auto f = read_write_file::open(io_service(), temp_dir() / "foo.txt");
		...
		qor_pp_assert_that(std::memcmp(buffer1, buffer2, 50) == 0);

		co_await f.read(50, buffer2, 50);
		qor_pp_assert_that(std::memcmp(buffer1 + 50, buffer2, 50) == 0);
	};
